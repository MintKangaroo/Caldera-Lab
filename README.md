...
**이것은 음성 결과이고, 규명되었습니다.** 탭 정책은 오라클과의 격차(0.30)를 회수하지 못하고
무정보 기준선에 머뭅니다. 두 극단을 하나의 Q 값으로 평균 낼 뿐, 이번 실행의 위험을 추정하지
못합니다.

원인은 state에 있습니다. 실행 중 위험의 유일한 증거는 뿌리의 실패인데, `degraded`는 **실행
전체를 통틀어 한 비트**입니다. 어떤 능력이 실패했는지 말하지 못하므로 뿌리의 실패를 담을 수
없습니다. 능력별 실패 이력을 담으면 되지만, 그건 state 공간을 다시 키웁니다 — 그 트레이드오프는
열어둡니다. `latent_risk_bounds`가 그 실험의 측정 도구입니다.
...
에피소드별 위험    -> 추정 못함 (-1%, 음성 결과). degraded가 한 비트라 뿌리 실패를 못 담음
