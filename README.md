...
값입니다.** 그래서 보상은 순서의 함수가 아니라 집합의 함수이고, 전량 실행에서는 모든 순서가
같은 값을 냅니다. 정책이 순서에 대해 배울 것이 없는 것은 버그가 아니라 보상 설계의 결과입니다.

선행 조건은 *가능한* 순서를 제약하지만, 가능한 순서들 사이의 우열은 만들지 않습니다. 순서가
의미를 가지려면 보상이 능력을 구분해야 합니다 — 이는 설계 결정이라 열어둡니다.
...
RL 순서 학습       -> 없음. 전량 실행 시 무작위 순서 300회 보상 편차 0.000 (보상이 집합 함수)
