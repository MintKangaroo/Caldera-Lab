...
즉 한 비트는 **위험들이 함께 움직이는 동안만** 충분합니다. 독립 위험을 구분하려면 위험당 한
비트가 필요한데, 이는 단일 위험의 교훈과 정반대입니다 — 거기선 두 번째 비트가 수렴만 늦췄죠.
어느 쪽이든 **비트를 늘릴지 말지는 위험이 상관돼 있는지에 달렸다**는 게 요점입니다. 수렴은
`tests/probe_multi_risk.py`에서 재현하고, CI에는 구조적 사실만 남깁니다.
...
수렴은 `tests/probe_nonstationary.py`에서 재현하고, CI에는 구조적 사실만 남깁니다.
