...
9. **보상이 순서를 구분하지 못함**: 측정 결과 전량 실행 시 무작위 순서 300회의 보상 편차가
   0.000입니다. 정보 이득이 novel/(novel+known)로 정규화되어 처음 실행하는 능력은 무엇이든
   같은 값이기 때문입니다. 선행 조건은 가능한 순서를 제약할 뿐 우열을 만들지 않으므로 RL이
   순서에 대해 배울 것이 없습니다. 보상이 능력을 구분하도록 바꿀지는 설계 결정입니다.
   (그 과정에서 발견한 별개 버그 3개는 수정 완료: 계획의 whitelist화, future 항의 전체 max,
   0 초기화.)
