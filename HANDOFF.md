...
2. ~~**LLM planner 계약 강화**~~: 완료(5-1 참고). 실제 키 검증도 완료 — `--planner claude`로
...
8-1. ~~**순차↔동시 전이**~~: 해결되어 있었습니다. 두 표현 모두 순차 학습 table을 동시 실행이
   사실상 전부 읽습니다. 이전 문서의 "38% -> 75%"는 outcome 성분이 `last_status`이던 시절
   수치이고, 그 성분을 `clean`/`degraded`로 바꾸면서 구조적 불일치가 사라졌습니다.

   측정 함정 하나: **epsilon을 0으로 두는 것만으로는 학습된 정책을 재지 못합니다.** 미측정
   (state, action)이 학습 중 일부러 매력적이므로 greedy 실행도 탐험이 됩니다. `bench`는
   평가 시 측정된 값만 보게 합니다(`_exploit_only`).

8-2. ~~**RL이 최적에 못 미침**~~: 해결. 원인은 미측정 값이 상수 2.0이었다는 것입니다(실제 Q는
   8 근처). 상수 대신 지금까지 측정된 최대값을 쓰게 하니 12000 에피소드에서 DP 최적에
   도달합니다. `optimism`은 이제 하한일 뿐입니다.
...
9-1. ~~**RL이 최적에 도달하지 못함**~~: 해결. 상태를 "어떤 능력을 배정했는지"에서 "어떤
   trait을 알고 예산이 얼마나 남았는지"로 바꾸니 12000 에피소드에서 DP 최적에 정확히
   도달합니다(이전 표현은 같은 지점에서 52.2%). 방문 상태 360 -> 55. 기본값은 `facts`이고
   `issued`도 `Coordinator(state_mode=...)`로 선택할 수 있습니다. 저장된 table의 지문에
   상태 모드가 포함되어 서로 섞이지 않습니다.

   (이전 기록) **RL이 최적에 도달하지 못함**: 2^11 부분집합 DP로 정확한 최적(7.8650)과 최악(7.2615,
...
이 샌드박스는 자연적으로 실패하지 않으므로 실패는 주입해야 합니다. 주입된 결과는
`injected: true`로 표시되어 컨테이너가 실제로 보고한 것과 구분됩니다.

```bash
caldera-lab run --fault-ability collect-process-list=0.6   # 능력별
caldera-lab run --fault-rate 0.2                           # 균일 (학습에는 무의미)
```

**균일한 실패율은 정책에 아무것도 가르치지 않습니다.** 선택과 무관한 실패는 선택으로 피할 수
없어서 Q 추정에 잡음만 더합니다(측정: -0.29). 능력별 실패율이라야 위험이 학습 가능해집니다
(측정: +0.98, 정책이 불안정한 뿌리를 가진 사슬을 뒤로 미룸).

실패를 넣으면 DP 최적 기준선이 성립하지 않습니다. `bench.bounds`는 무결점 경우의 기준이고,
실패 아래 비교는 `bench.under_faults`(여러 시행 평균)를 씁니다.

### 재시도

`--max-attempts N`(기본 1). 실패한 능력은 아무것도 만들지 못했으므로 소모되지 않았고, 예산
안에서 다시 배정될 수 있습니다. 재시도는 한 단계를 쓰므로 결정입니다.

측정: 실패율이 같은 두 능력 중 3단 사슬의 뿌리는 실패 후 81.2% 재시도, 뒤에 아무것도 없는
잎은 0.0%. `degraded` state가 이 구분을 담으며, 짝이 맞는 state 21개 중 8개에서 최선 행동이
`clean`일 때와 다릅니다.
...
- **고정 숨은 위험**: 운영자가 `--fault-ability`로 고정. state에 한 번도 안 들어가지만 정책이
  관찰로 학습합니다(위험한 사슬의 Q 값이 자기 실패로 할인). 이미 됩니다.
- **에피소드별 위험**: 실패율을 매 실행 새로 뽑음. 카나리아 없으면 정책이 추정하지 못합니다
  (-1%). 원인은 처음 생각한 "state가 못 담아서"가 **아닙니다** — 생산자별 실패 비트를 넣어봐도
  -1% 그대로였습니다. 진짜 이유는 관찰=비용입니다: 위험을 보려면 위험한 뿌리를 실행해야 하고,
  실패하면 뒤 사슬이 fact 없이 죽어 바꿀 결정이 없습니다. 정보가 늦게 도착합니다.
  카나리아(같은 rate로 실패하는 값싼 잎)를 주면 관찰과 비용이 분리되고, 기존 degraded 한 비트로
  오라클에 도달합니다(101%). 실패 비트를 추가하면 상태 공간만 커져 -51%로 나빠집니다.
