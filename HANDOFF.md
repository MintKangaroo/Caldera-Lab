...
- **에피소드별 위험**: 실패율을 매 실행 새로 뽑음. 탭 정책은 **추정하지 못합니다**(무정보
  기준선에 붙음, -1%). 실행 중 위험의 유일한 증거는 뿌리 실패인데 `degraded`가 실행 전체
  한 비트라 어떤 능력이 실패했는지 못 담습니다. 능력별 실패 이력을 넣으면 되지만 state 공간이
  커집니다 — 트레이드오프로 열어둠.

`bench.latent_risk_bounds(catalog, outcomes, ability, rates)`가 세 기준선(무정보/오라클/탭)을
계산합니다. `position < 0`이면 정책이 한 순서에 고정하는 것보다도 못하다는 뜻입니다.
