...
9. ~~**보상이 순서를 구분하지 못함**~~: 깊이 보상으로 해결. 능력마다 depth(선행 발견 사슬의
   ...
   함수지만, 할인된 수익은 순서에 따라 달라집니다(편차 0.289). 학습 0% -> 59%.
   (그 과정에서 발견한 별개 버그 3개도 수정: 계획의 whitelist화, future 항의 전체 max,
   0 초기화.)

9-1. **RL이 59%에서 정체**: 최적에 도달하지 못합니다. 상태가 어떤 fact를 아는지가 아니라
   어떤 능력을 배정했는지만 담고, 4096개 상태 대비 탐험이 얇은 것이 유력한 원인입니다.
   측정은 되어 있으므로 상태 표현이나 탐험 전략을 바꿔가며 비교할 수 있습니다.
